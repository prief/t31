  ...
  methods: {
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          instance.post(`/${this.namespace}/${this.entityName}/save`, JSON.stringify(this.formData, { arrayFormat: 'repeat' })).then(response => {
    ...
    // 根据ID加载数据
    get (id) {
      instance.get(`/${this.namespace}/${this.entityName}/info/` + id).then(response => {
