...
export default {
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.trainOrder(params.row.id, params.row.trainNumber, params.row.fromStation, params.row.toStation, params.row.trainDate, params.row.numberId, params.row.startTime)
                  }
                }
              }, '购买车票'),
              h('Button', {
  ...
  methods: {
    ...
    // 下单
    trainOrder (id, trainNumber, fromStation, toStation, trainDate, numberId, startTime) {
    ...
    query () {
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          instance.post(`/${this.namespace}/${this.entityName}/list-page`, JSON.stringify(this.formData)).then(response => {
    ...
    handleSearchFromStation (value) {
      if (value) {
        instance.get(`/train/scheduler/from-stations?fromStation=${value}`).then(response => {
    ...
    handleSearchToStation (value) {
      if (value) {
        instance.get(`/train/scheduler/to-stations?toStation=${value}`).then(response => {
