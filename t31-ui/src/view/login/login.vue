      ...
      <Card icon="log-in" title="欢迎登录" :bordered="false">
        <div class="form-con">
          ...
          <Form ref="form" :model="formData" :rules="ruleValidate">
            ...
            <FormItem>
              <Button @click="handleSubmit('form')" type="primary" >登录</Button>
              <Button @click="go2Register()" type="primary" style="margin-left: 8px">注册</Button>
            </FormItem>
          </Form>
        </div>
      </Card>
  ...
  methods: {
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          instance.post(`/admin/user/login`, JSON.stringify(this.formData, { arrayFormat: 'repeat' })).then(response => {
            ...
            if (!menus) {
              instance.get(`/admin/user/select-user-menu`).then(response => {
