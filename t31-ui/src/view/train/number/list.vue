...
export default {
  ...
  methods: {
    ...
    handleSearchStartStation (value) {
      if (value) {
        instance.get(`/train/number/start-stations?startStation=${this.formData.startStation}`).then(response => {
    ...
    handleSearchEndStation (value) {
      if (value) {
        instance.get(`/train/number/end-stations?endStation=${this.formData.endStation}`).then(response => {
